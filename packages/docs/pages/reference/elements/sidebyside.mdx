...
The `<sidebyside>` tag provides a container in which to arrange items horizontally rather than vertically, such as three images in a row or blocks of text with associated commentary next to them.
...
<sidebyside widths = "30% 60%" margins="5% 5%">
  ...
  <p>
    We could give lots of examples, but instead it might be best to just experiment.
     Error-checking is very robust, so it is hard to get it too wrong.
  </p>
</sidebyside>
</FRAGMENT>
```
...
sidebyside widths="25% 65%">
  <figure xml:id="fig-factor-through-quotient">
